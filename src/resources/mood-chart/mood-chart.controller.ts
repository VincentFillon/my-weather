...
import { AuthenticatedRequest } from 'src/utils/authenticated-request';
...
export class MoodChartController {
  ...
  async getMoodChartData(@Req() req: AuthenticatedRequest, @Query('days') days: number): Promise<MoodChartDataDto[]> {
    const nbDays = days != null ? +days : 7; // Si aucun nombre de jours n'est fourni, utiliser 7 par défaut
    return this.moodChartService.getMoodChartData(req.user.sub, req.user.activeGroup, nbDays);
  }
}